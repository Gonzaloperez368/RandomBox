...
- [~] FASE 9 — Testing y correcciones (primera pasada por las 6 herramientas hecha; ver "Estado actual")
...
FASE 9 en curso: primera pasada de testing por las 6 herramientas (caso normal, límite, inválido, vacío y repetición de cada una, corriendo la app real en el navegador). Se encontraron y corrigieron dos problemas:

- **Bug real en Torneos (llaves con muchos más "byes" que participantes)**: `avanzarGanador()` solo hacía avanzar los pases libres UN nivel (de la ronda 1 a la ronda 2). Con una llave grande y pocos participantes reales (ej. 16 lugares con solo 2 nombres cargados), alguien podía tener que pasar libre varias rondas seguidas porque toda su mitad de la llave estaba vacía — y como el avance no cascadeaba, esa persona quedaba esperando para siempre contra un "Por definir" que nunca se iba a completar, y la Final nunca se resolvía sola. Ahora `avanzarGanador()` es recursivo: después de poner a alguien en la ronda siguiente, revisa si ESE partido también quedó resuelto solo (por ejemplo, contra una rama que también está completamente vacía) y si es así sigue avanzando, cuantas rondas hagan falta. También se generaliza `generarLlaveDeTorneo()` para que un cruce de ronda 1 con los dos lados vacíos (`null`/`null`, no solo un lado) dispare esta cascada desde el arranque. `crearElementoDePartido()` y `crearElementoDePartidoRonda1()` ahora distinguen tres estados en vez de dos: alguien real, una rama que nunca va a tener a nadie (`null`, se muestra "(sin participantes)" si el cruce entero está vacío, o "(nadie)" si el otro lado sigue esperando una decisión real en otra parte de la llave) y "Por definir" (`undefined`, esperando de verdad). `deshacerAvanceDeGanador()` no necesitó cambios: ya deshacía en cascada correctamente.
- **"Cantidad" sin tope en Generador de números y Selector de nombres**: a diferencia del Lanzamiento de moneda (que ya tenía `MAXIMO_LANZAMIENTOS`), estas dos herramientas no tenían ningún límite superior en la cantidad a generar/elegir. Probado en el navegador: pedir 1000 números (o 5000 elecciones) cuelga la pestaña varios segundos porque cada resultado tiene su propio temporizador de animación corriendo en simultáneo (o, en Revelación, se crean todos los casilleros de golpe). Se agregó `MAXIMO_NUMEROS_A_GENERAR` y `MAXIMO_A_ELEGIR` (ambos en 100), mismo criterio que `MAXIMO_LANZAMIENTOS` — probado que 100 sí revela sin trabarse.

No se pudo probar en vivo la animación de la ruleta del Selector de nombres en esta pasada (usa `requestAnimationFrame`, que el navegador pausa cuando el panel de vista previa queda en segundo plano en este entorno) — se revisó el código a mano y no se encontró ningún problema; el usuario ya la había probado a fondo en FASE 4 ("probé todos los casos, funciona bien").

Próximo paso: seguir FASE 9 (falta revisar a fondo más casos de Torneos — swap/rename combinados con byes en cascada, por ejemplo) o pasar a FASE 10 si el usuario prefiere — preguntar, no asumir.
